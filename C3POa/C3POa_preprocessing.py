...
def read_and_filter_fastq(input_file):
    read_dict, read_iter = {}, {}
    iterator, folder = 0, 0
    ### NEW CODE: prevents overwriting of output dir when preprocessing multiple fastq files
    s_dirs = [name for name in os.listdir(output_path) if os.path.isdir(os.path.join(output_path, name))]
    s_dirs_num = [int(name) for name in s_dirs]
    if s_dirs_num:
        folder = max(s_dirs_num)
    ###
...
def run_blat(path, reads):
    ...
    ### NEW CODE: used to be -minScore=15, no -tileSize (default set to 11), no -fine, no -oneOff
    os.system('%s -noHead -stepSize=2 -t=DNA -q=DNA -minScore=9 -tileSize=7 -fine -oneOff=1 \
...
def parse_blat(path):
    ...
    for line in open(alignment_file):
        ...
        ### NEW CODE: changed score threshold from 50 to 9
        if gaps < 50 and score > 9: # Looks for unspliced quality alignment
            if strand == '+':
